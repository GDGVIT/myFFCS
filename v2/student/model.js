...
return kuzhanthaidata(regno,courseId).flatMap((x)=>{
  ...
  console.log(x);
   if(validator(x.course.slots.split('+'),x.user.newAllotedSlot2)){
     model.update({_id:x.user._id},{$push:{newAllotedCourse2:x.course._id},$pushAll:{newAllotedSlot2:x.course.slots.split('+')}})
...
return courseDetail({id:id,data:regno}).flatMap((x)=>{
  return Kefir.stream(function (emitter) {
    model.update({regno:regno},{$pull:{newAllotedCourse2:id},$pullAll:{allotedSlot:x.course.slots.split('+')}}).exec(function (err,data) {
      if(err){
